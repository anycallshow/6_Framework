...
memberPw.addEventListener("input", () => {
    ...
    // 정규 표현식을 이용해서 비밀번호 유효성 검사
    // 1) 영어,숫자,특수문자(!,@,#,-,_) 6~20글자
    const regEx = /^[A-Za-z\d\!\@\#\_\-]{6,20}$/;

    // 2) 입력 받은 비밀번호와 정규식 일치 여부 판별
    if(regEx.test(memberPw.value)){ // 유효한 경우
        checkObj.memberPw = true; // 유효 O
        ...
        if(memberPwConfirm.value.trim().length == 0){
            ...
            if(memberPw.value == memberPwConfirm.value){
                ...
                checkObj.memberPwConfirm = true; // 유효 O
                ...
                checkObj.memberPwConfirm = false; // 유효X
            }
...
memberPwConfirm.addEventListener("input", () => {
    ...
    if(checkObj.memberPw){ // 비밀번호가 유효하게 작성된 경우
...
memberNickname.addEventListener("input", ()=>{
    ...
    if(memberNickname.value.trim().length == 0){
        ...
        checkObj.memberNickname = false; // 빈칸 == 유효X
...
memberTel.addEventListener("input", ()=>{
    ...
    if(memberTel.value.trim().length == 0){
        ...
        checkObj.memberTel = false; // 빈칸 == 유효X
    ...
    if(regEx.test(memberTel.value)){
        telMessage.innerText = "유효한 형식 입니다.";
        telMessage.classList.remove("error");
        telMessage.classList.add("confirm");

        checkObj.memberTel = true; // 유효 O

    }else{
        telMessage.innerText = "유효하지않은 형식 입니다.";
        telMessage.classList.remove("confirm");
        telMessage.classList.add("error");
        
        checkObj.memberTel = false; // 유효 X
    }
})
