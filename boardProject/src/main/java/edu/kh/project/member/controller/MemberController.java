...
// @RequestMapping(value="요청 주소", mathod=RequestMethod.GET/POST)
// -> GET / POST 방식을 구분
...
@SessionAttributes("loginMember") // Model의 이름(key)을 적으면 session으로 추가
public class MemberController {
	...
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		// 파라미터 전달 방법 3 : @ModelAtteribute를 이요한 방법
		
		// - DTO(또는 VO)와 같이 사용하는 어노테이션
		
		// - 전달 받은 파라미터의 name 속성 값이
		//	 같이 사용되는 DTO의 필드명과 같다면
		//	 자동으로 setter를 호츨해서 필드에 값을 세팅
		...
		// *** @ModelAttribute를 이용해 값이 필드에 셋팅된 객체를
		//	   "커맨드 객체"라고 한다. ***
	...
	public String logout(SessionStatus status, HttpSession session) {
		...
		// -> SessionStatus.setComplete() [Spring에서는 이거 사용!]
